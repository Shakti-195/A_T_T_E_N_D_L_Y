...
# -*- coding: utf-8 -*-
"""
Simplified authentication routes — no email OTP verification required.
...
def register():
    ...
    if request.method == 'POST':
        try:
            ...
            # Direct login after registration (no email verification)
    ...
    if request.method == 'POST':
        if g.user.role == 'teacher':
            ...
            institution = Institution.query.filter(
                func.upper(Institution.institution_code) == inst_code.upper()
            ).first()
            if not institution:
                flash('Invalid Institution ID.', 'danger')
                return redirect(url_for('auth.join_institution'))
            ...
            try:
                ...
                flash('Successfully joined institution and linked your record!', 'success')
                return redirect(url_for('dashboard.dashboard'))

            except Exception as e:
                db.session.rollback()
                flash('An error occurred while linking your student record.', 'danger')
                return redirect(url_for('auth.join_institution'))
...
def forgot_username():
    if request.method == 'POST':
        ...
        if user:
            send_username_email(user)
        flash('If an account exists, your username has been sent.', 'info')
        return redirect(url_for('auth.login'))
    return render_template('auth/forgot_username.html')
...
def get_batches_for_institution(institution_code):
    institution = Institution.query.filter(
        func.upper(Institution.institution_code) == institution_code.upper()
    ).first()
    if not institution:
        return jsonify({'error': 'Institution not found'}), 404

    batches = [{'id': b.id, 'name': b.name} for b in institution.class_batches]
    return jsonify(batches)
...
def get_students_in_batch(batch_id):
    students = Student.query.filter_by(class_batch_id=batch_id, user=None).all()
    student_list = [{'id': s.id, 'name': s.name, 'student_id': s.student_id} for s in students]
    return jsonify(student_list)
