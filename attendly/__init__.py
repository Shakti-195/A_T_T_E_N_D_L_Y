...
This file contains the application factory, create_app(), which is responsible
for initializing the Flask application, its extensions, and its blueprints.
Version: 2.1 - Brevo SMTP Integrated + Email & Scheduler Support
"""
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = (
        database_url or f"sqlite:///{os.path.join(project_root, 'attendance.db')}"
    )
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {
        'pool_pre_ping': True,
    ...
    app.config['MAIL_USERNAME'] = os.environ.get('MAIL_USERNAME', 'your_registered_brevo_email@example.com')
    app.config['MAIL_PASSWORD'] = os.environ.get('BREVO_SMTP_KEY')  # 🔐 Use Brevo SMTP key
    app.config['MAIL_DEFAULT_SENDER'] = os.environ.get('MAIL_DEFAULT_SENDER', 'Attendly <9a8252001@smtp-brevo.com>')
    ...
    @click.argument('recipient')
    def test_email_command(recipient):
        """Send test email using Brevo SMTP."""
        try:
            from .email import send_test_email
            if send_test_email(recipient):
                click.echo(f"✅ Test email sent to {recipient}. Check inbox!")
            else:
                click.echo("❌ Failed to send email. Check logs.")
        except Exception as e:
            logger.error(f"Test email error: {str(e)}")
            click.echo(f"❌ Error: {str(e)}")
    ...
    # --- Log Startup Info ---
    logger.info("=" * 70)
    logger.info("🚀 ATTENDLY APPLICATION STARTED (with Brevo SMTP)")
    logger.info("=" * 70)
    logger.info(f"Database: {app.config['SQLALCHEMY_DATABASE_URI']}")
    logger.info(f"Mail Server: {app.config['MAIL_SERVER']}:{app.config['MAIL_PORT']}")
